                                      ...
                                      <table id="product-table_wrapper" class="table table-striped table-hover products dt-responsive" cellspacing="0" width="100%">
                                              ...
                                              <tbody>
                                                @foreach($orders as $order)                                                  
                                                    ...
                                                    <td> <a href="{{route('frenchise-order-invoice',$order->id)}}">{{sprintf("%'.08d", $order->id)}}</a></td>
                                                    <td> {{$order->qty}}</td>
                                                    <td> {{$order->currency_sign}}{{ round($order->price * $order->currency_value , 2) }}</td>
                                                    <td> {{$order->method}}</td>
                                                      <td>
                                            ...
                                            ">{{ucfirst($order->status)}}
                                                <span class="caret"></span></button>
                                                        <ul class="dropdown-menu">
                                                            <li><a href="javascript:;" data-href="{{route('frenchise-order-status',['id1' => $order->id, 'status' => 'processing'])}}" data-toggle="modal" data-target="#confirm-delete">Processing</a></li>
                                                            <li><a href="javascript:;" data-href="{{route('frenchise-order-status',['id1' => $order->id, 'status' => 'completed'])}}" data-toggle="modal" data-target="#confirm-delete">Completed</a></li>
