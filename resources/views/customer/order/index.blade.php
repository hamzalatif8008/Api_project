                                      ...
                                      <table id="product-table_wrapper" class="table table-striped table-hover products dt-responsive" cellspacing="0" width="100%">
                                              ...
                                              <tbody>
                                                @foreach($orders as $order)
                                                    ...
                                                    <td> <a href="{{route('customer-order-invoice',$order->id)}}">{{sprintf("%'.08d", $order->id)}}</a></td>
                                                    <td> {{$order->totalQty}}</td>
                                                    <td> {{$order->currency_sign}}{{ round($order->pay_amount * $order->currency_value , 2) }}</td>
                                                    <td> <input type="hidden" value="{{$order->customer_email}}"> {{$order->method}}</td>
                                                    @if($order->method==="pending")
                                                    <td><a href="{{route('paymentpage',$order->id)}}" class="btn btn-primary product-btn"><i class="fa fa-check"></i> Proceed Order</a> </td>
                                                    @else
                                                     <td><a href="{{route('customer-order-show',$order->id)}}" class="btn btn-primary product-btn"><i class="fa fa-eye"></i> View Details</a></td>
                                                    @endif 
                                                  </tr>
                                                  @endforeach
                                                  </tbody>
